...
use CargoTruckingSpace\TemplateFunctions\Post_Elements;
...
if ( is_search() && ! have_posts() ) {
	echo '<div class="' . $parent_class . '__no-posts">' .
		'<h3 class="' . $parent_class . '__no-posts-title">' . esc_html__( 'Nothing found', 'cargo-trucking' ) . '</h3>' .
		'<div class="' . $parent_class . '__no-posts-description">' .
			'<p>' . esc_html__( 'Sorry, no posts matched your criteria. Please try another search.', 'cargo-trucking' ) . '</p>' .
			'<p>' . esc_html__( 'You might want to consider some of our suggestions to get better results:', 'cargo-trucking' ) . '</p>' .
			'<ul>' .
				'<li>' . esc_html__( 'Check your spelling.', 'cargo-trucking' ) . '</li>' .
				'<li>' . esc_html__( 'Try a similar keyword, for example: tablet instead of laptop.', 'cargo-trucking' ) . '</li>' .
				'<li>' . esc_html__( 'Try using more than one keyword.', 'cargo-trucking' ) . '</li>' .
			'</ul>' .
		'</div>' .
	'</div>';
}
